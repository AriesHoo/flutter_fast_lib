...
///错误视图--如果无全局、无局部则使用该内置效果
class FastErrorStateWidget extends StatelessWidget {
  final String? message;
  final GestureTapCallback? onPressed;

  const FastErrorStateWidget({
    Key? key,
    this.message,
    this.onPressed,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return FastStateWidget(
      ...
      message: Text(message ?? FastManager.getInstance().textMixin.stateErrorMessage),
      button: TextButton(
        onPressed: onPressed,
